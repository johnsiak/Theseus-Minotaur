...
public class MinMaxPlayer extends Player{ // this class simulates the player that uses the MinMax Algorithm
    ...
    public int chooseMinMaxMove(Node root) {
        ...
        moveScore = max;			//we store the evaluation in the moveScore variable
	...
	public void createOpponentSubtree(int currentPos, int opponentCurrentPos, Node parent, int depth, double parentEval) {
		...
		for(int dice: dices) {
			...
			//we find the evaluation according to the player
			double evaluation;
			if(playerId == 1) evaluation = parentEval - evaluate("Minotaur", opponentCurrentPos, dice, currentPos, opponentBoard); //if the player is Theseus (the opponent is Minotaur)
			else evaluation = parentEval - evaluate("Theseus", opponentCurrentPos, dice, currentPos, opponentBoard);
			
			int[] m = move(opponentCurrentPos, dice, opponentBoard, false, false, false);  //we simulate the move in the clone board	
	...
	public void createMySubtree(int currentPos, int opponentCurrentPos, Node root, int depth) {
		...
		for(int dice: dices) {
			...
			//we find the evaluation according to the player
			double evaluation;  
			if(playerId == 1) evaluation = evaluate("Theseus", currentPos, dice, opponentCurrentPos, childBoard); //if the player is Theseus
			else evaluation = evaluate("Minotaur", currentPos, dice, opponentCurrentPos, childBoard); 
			
			int[] m = move(currentPos, dice, childBoard, false, true, false);	//we simulate the move in the clone board
	...
	public int getNextMove(int currentPos, int opponentCurrentPos) {
        ...
        if(playerId == 1) {
            ...
            finalMove = move(currentPos, dice, board, true, false, false); //we set both realMove as true and gatherSup as false because it is a real move and we don't want Minotaur to gather supplies, we set randomMove as false because it's not a random move
        }
        int el0 = dice;
