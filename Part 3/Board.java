...
public class Board {
	...
	public Board(Board theBoard) {
		...
		tiles = new Tile[N * N];
		supplies = new Supply[S];
	    for(int i = 0; i < theBoard.getTiles().length; i++) 
	    	tiles[i] = new Tile(theBoard.getTiles()[i]);
        for(int i = 0; i < theBoard.getSupplies().length; i++) 
            supplies[i] = new Supply(theBoard.getSupplies()[i]);
    }
